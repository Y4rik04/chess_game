...
void Board::setBoard()  //функци€ создани€ доски
{
	...
	for (int i = 0; i < 8; i++) //вставл€ем пешки на доску
	{
		square[i][1].setPieceAndColor(PAWN, WHITE);
		square[i][6].setPieceAndColor(PAWN, BLACK);

	}
	for (int i = 2; i < 6; i++) //заполн€ем пустие клетки
	{
		for (int j = 0; j < 8; j++)
...
void Board::printBoard() //функци€ принта доски
{
...
bool Board::moveKing(Square* thisKing, Square* thatSpace) //движение корол€
{
...
bool Board::moveQueen(Square* thisQueen, Square* thatSpace) //движение ферз€
{ 
...
bool Board::moveKnight(Square* thisKnight, Square* thatSpace) //движение кон€
{
